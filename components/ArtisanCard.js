    ...
    >
      <div className="relative aspect-[16/9] w-full bg-gradient-to-br from-slate-100 to-slate-200 overflow-hidden img-zoom">
        <img
          ...
          loading="lazy"
        />
        <div className="absolute top-3 right-3 bg-gradient-to-r from-brand to-emerald-600 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-md animate-slide-down">{t('handicraft')}</div>
      </div>
      <div className="p-4 bg-white">
        ...
        <div className="mt-3 inline-block text-xs font-semibold text-brand bg-brand/10 px-2 py-1 rounded-md group-hover:bg-brand/20 transition-colors">{t('viewCatalog')}</div>
      </div>
    </Link>
  );
}
