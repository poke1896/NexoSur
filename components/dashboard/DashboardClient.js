        ...
        <form onSubmit={addOrUpdateProduct} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {editingId && (
            <div className="sm:col-span-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <label className="text-xs font-semibold text-blue-900 block mb-1">ID del producto (no editable)</label>
              <input className="w-full border border-blue-300 rounded px-2 py-2 bg-white text-gray-700 font-mono text-sm cursor-not-allowed" placeholder="ID" value={form.id} disabled />
            </div>
          )}
